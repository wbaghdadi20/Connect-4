...
class Gameboard:
    ...
    def __init__(self, ai):
        ...
        self.screen = self.setup(ai)

    def setup(self, ai):
        screen = pg.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
        caption = 'Connect 4: ' + ai
    ...
    def display_winner(self):
        if not self.empty_spaces():
            self.display("Draw !!", 100, 10, LIGHT_GREY)
        elif self.winning_move(AI):
            self.display("AI Wins !!", 120, 10, YELLOW)
        else:
            self.display("Player Wins !!", 30, 10, RED)
